...
// The customer fields collected by the form. Each entry describes how to
...
// Small inline loading spinner (animated). Size/colour via className override.
  ...
  return (
    <>
      ...
      {/* AI insight — loaded on demand. The probability above always comes from the
          ML model; the AI here only explains it and suggests actions in words. */}
        ...
        {!insight && !insightError && (
          <>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-80"
            >
              {insightLoading ? (
                <>
                  <Spinner />
            ...
            {!insightLoading && (
              <p className="mt-2 text-xs text-slate-400">
                Turns this customer’s prediction and key factors into a plain-English
                explanation and suggested actions. The churn % is never changed.
              </p>
            )}

            {/* Animated skeleton: a clear visual cue while the AI responds. */}
            {insightLoading && (
              <div className="mt-4 space-y-2" aria-live="polite" aria-busy="true">
                <div className="h-3 w-3/4 animate-pulse rounded bg-slate-200" />
                <div className="h-3 w-full animate-pulse rounded bg-slate-200" />
                <div className="h-3 w-5/6 animate-pulse rounded bg-slate-200" />
                <p className="flex items-center gap-2 pt-1 text-xs text-violet-600">
                  <Spinner className="h-3.5 w-3.5 text-violet-600" />
                  Analysing the prediction and drafting recommendations…
                </p>
              </div>
            )}
          </>
  ...
  return (
    <div className="space-y-3 text-sm">
      <div className="flex items-center gap-2">
        ...
        >
          {isGemini ? "AI-generated" : "Rule-based summary"}
        </span>
